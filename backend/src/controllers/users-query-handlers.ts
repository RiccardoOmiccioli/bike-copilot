...
import {BikeCopilotDatabase, UserDocument} from "../data/bike-copilot-database";
...
export class UsersQueryHandlers {
    ...
    public static readonly findAll: RequestHandler = (req: Request, res: Response) => {
        BikeCopilotDatabase.Singleton.Users
            .find({},{}).exec()
    ...
    public static readonly insertOne: RequestHandler = (req: Request, res: Response) => {
        Require.fields(req.body, "name", "email", "password", "role").then(requirementSatisfied => {
            BikeCopilotDatabase.Singleton.Users    // Search for a user with the same email
                ...
                .then(alreadyRegisteredUser => {
                    if (alreadyRegisteredUser !== null){
                        ...
                        BikeCopilotDatabase.Singleton.Users    // Add the requested user to the database
                            .insertMany([newUser])
                            .then(sendJson(req, res), sendError(req, res));
                    }
                }, sendError(req, res))
    ...
    public static readonly updateProfile: RequestHandler = (req: Request, res: Response) => {
        BikeCopilotDatabase.Singleton.Users    // Search for a user with the same id or the same new email
